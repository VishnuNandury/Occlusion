...
class particle:
    def __init__(self, x,y,width, height, N,color):
        ...
        if(N <= 0 or N>(width*height)): 
            raise ValueError('The ParticleFilter class does not accept a value of N which is <= 0 or >(widht*height)')
        self.particles = np.empty((N, 2))
        self.particles[:, 0] = uniform(x, x+width, size=N) #init the X coord
        self.particles[:, 1] = uniform(y, y+height, size=N) #init the Y coord
    ...
    def resample(self, method='stratified'):
    ...
    def drawParticles(self, frame, color=[0,0,255], radius=2):
        ...
        for x_particle, y_particle in self.particles.astype(int):
            cv.circle(frame, (x_particle, y_particle), radius, self.clr, -1) #RED: Particles
